...
class AuthController extends Controller
{
    ...
    public function postlogin(Request $request){
        ...
        if (Auth::attempt($re,$ingat)) {
            ...
            return redirect('/login')->with('status', 'รหัสผ่านไม่ถูกต้อง');
        }
    }
    //register view
    ...
    if ($request->agreeterm == true) {
            ...
            Profile::create($request->all());
                ...
                return redirect('/register')->with('status', 'รหัสผ่านไม่ตรงกัน');
            }
    }else {
            return redirect('/register')->with('status1', 'คุณต้องตรวจสอบข้อกำหนดก่อน');
    }
    }
